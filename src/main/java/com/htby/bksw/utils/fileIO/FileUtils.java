package com.htby.bksw.utils.fileIO;

import com.htby.bksw.utils.config.ConfigInfo;
import lombok.Data;
...
public class FileUtils {
    ...
    public boolean saveLoginInfo( ConfigInfo configInfo){
        ...
        file_name = file_name == null ? "collectConfig.txt" : file_name;
        log.info("配置文件的 -- 保存 -- 路径：{}",file_path);
        log.info("配置文件的 -- 保存 -- 名称：{}",file_name);
        String system_dir = System.getProperty("user.dir");
        system_dir = system_dir.substring(system_dir.length()-13,system_dir.length()-1).equals("iots_collect") ? system_dir : (system_dir + "/iots_collect");
        ...
        try {
            ...
            log.info("保存的配置信息是：{}",configInfo.toString());
            return true;
        } catch (Exception e) {
