...
public class LoggerUtil {
    ...
    public List<String> getChild( File file, List<String> list,String identify,boolean isupload){
        ...
        for (int i = 0; i < tempList.length; i++) {
            if(isupload){
                if (tempList[i].isFile()&&tempList[i].getName().indexOf(identify)==-1&&tempList[i].getName().indexOf("spring.log")==-1) {
                    list.add(tempList[i].toString());
                    log.info("文     件：{}",tempList[i]);
                }
            }else{
                if (tempList[i].isFile()&&tempList[i].getName().indexOf(identify)>0&&tempList[i].getName().indexOf("spring.log")==-1) {
                    list.add(tempList[i].toString());
                    log.info("文     件：{}",tempList[i]);
                }
            }
