package com.htby.bksw.logger.utils;
...
public class RedisUtils {
	...
	public static void main(String[] args) {
		RedisUtils redisUtils= new RedisUtils("39.105.159.177",6379,"123456");
		redisUtils.set("bb","bb1");
		redisUtils.get("bb");
	...
	public RedisUtils() {
		...
		try {
			...
			String host = props.getProperty("spring.redis.host");
			log.info("redis host: {}",host);
			String password = props.getProperty("spring.redis.password");
			log.info("redis password: {}",password);
			int port = Integer.valueOf(props.getProperty("spring.redis.port","6379"));
			log.info("redis port: {}",port);
			GenericObjectPoolConfig poolConfig= new GenericObjectPoolConfig();
			jedisPool= new JedisPool( poolConfig, host, port,4000, password);
			this.jedisPool = jedisPool;
		} catch (IOException e) {
			log.error("redis 连接异常，异常信息为：{}",e.getMessage());
		}
	}
